...
@SQLDelete(sql = "UPDATE member SET member_status = 'MEMBER_QUIT', nickname = CONCAT('WYWdel', member_id), quitted_at = CURRENT_TIMESTAMP WHERE member_id = ?") // 탈퇴된 회원의 경우 del + memberId 형식으로 닉네임을 변경해줌으로 기존 닉네임의 점유권을 상실시킨다.
