...
//특수문자의 상한선을 확실히 정할 것 ㅇㅋ?
    ...
    public static class Post {
        ...
        @Pattern(regexp = "^(?!WYWdel\\d+$)[A-Za-z0-9가-힣]*$", message = "닉네임은 영문, 숫자, 한글만 허용됩니다.") // (?!WYWdel\d+$) 이 패턴을 추가함으로써 탈퇴된 회원들만의 닉네임 패턴인 WYWdel + 숫자 조합으로 다른 유저가 쉽사리 사용하지 않을만한 패턴을 이용했고 또 확실히 사용하지 못하게 block했다.
        @Size(min = 1, max = 16)
    ...
    public static class Patch {
        @Pattern(regexp = "^(?!WYWdel\\d+$)[A-Za-z0-9가-힣]*$", message = "닉네임은 영문, 숫자, 한글만 허용됩니다.")
        @Size(min = 1, max = 16)
