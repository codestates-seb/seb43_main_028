  ...
  useEffect(() => {
    ...
    function drawLines() {
      if (context && walkLog) {
        ...
        const minLng = Math.min(...walkLog.coordinates.map(coord => coord.lng))
        const maxLng = Math.max(...walkLog.coordinates.map(coord => coord.lng))
        const minLat = Math.min(...walkLog.coordinates.map(coord => coord.lat))
        const maxLat = Math.max(...walkLog.coordinates.map(coord => coord.lat))
        ...
        context.beginPath()
        for (let i = 0; i < walkLog.coordinates.length; i += 1) {
          const { lat, lng } = walkLog.coordinates[i]
